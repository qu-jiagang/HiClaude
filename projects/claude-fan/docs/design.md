...
- **风扇安装**：140 mm 风扇嵌在主体中央，大圆形开口；前盖、风扇和后壳用四颗 M4 风扇螺丝夹紧。
...
| 风扇外框 | 140 × 140 mm |
| 风扇厚度 | 26 mm |
| 安装孔距 | 124.5 mm（通用 140 mm PC 风扇孔距，估算） |
| 前盖螺丝孔 | 4.6 mm 通孔 + 8.8 × 2.2 mm 沉头/杯头让位 |
| 后壳螺丝柱 | 13 mm 外径，3.2 mm 打印导孔，供 M4 风扇螺丝自攻 |
| 中央开口直径 | 132 mm |
| 外壳主体宽度 | 176 mm |
| 外壳主体高度 | 156 mm（含头顶凸台 142+ 凸台） |
| 右侧走线槽 | 约 12 × 126 × 4.2 mm（按照片估算） |
| 控制器外壳 | 45.00 × 26.78 × 22.60 mm（直接导入 `标准A_快充A_3PIN版.STEP`） |
| 头顶凸台 | 104 × 32 mm（保留为外形/可选小线槽） |
| 固定仰角 | 15° |
...
当前版本默认风扇放入 `rear_shell` 的方形腔内，然后用四颗 M4 PC 风扇螺丝从 `front_cover` 正面穿过风扇安装孔，拧入 `rear_shell` 的 3.2 mm 导孔螺丝柱。螺丝柱已向后延伸进后壳底板，切片时是 `rear_shell` 的一体实体。

导入的控制器外壳作为独立 `control_pod` 摆在右侧，与后壳侧边接触；实物固定可用环氧胶、泡棉胶或后续追加小压片。

### 步骤

1. 把 FD14 风扇放入 `rear_shell` 的方形腔，并让四角安装孔对齐后壳螺丝柱。
2. 风扇线束沿 `rear_shell` 右侧边梁的开放走线槽下行，4-pin 插头和余线放入下方缓冲腔，三处压线桥用于轻压线束。
3. 线束从侧向出线口接到右侧 `control_pod`；该零件保持参考 STEP 的原始两件式外壳细节。
4. `front_cover` 扣到 `rear_shell` 前面，用四颗 M4 风扇螺丝从前盖穿过风扇后锁入后壳螺丝柱。
5. `top_spine` / `control_pod` / `wedge_feet` / `rear_heel` 用环氧或泡棉胶固定到 `rear_shell`。
